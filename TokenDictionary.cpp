...
TokenDictionary::TokenDictionary(){
    ...
    /// Operators:
    dictionary->addWord(new Assignment());
...
bool TokenDictionary::hasNext(char c) const {
    return cursor->getChild(c) != nullptr;
}

void TokenDictionary::reset() {
    cursor = dictionary->getRoot();
    DEBUG_PRINT("reset dictionary\n");
}

Token * TokenDictionary::getData() const {
    return cursor->getData();
}

Token * TokenDictionary::searchNextChar(char c) const {
...
Token * TokenDictionary::searchString(const std::string & s){
    reset();
    unsigned i = 0;
...
void TokenDictionary::addToken(Token * op) {
    dictionary->addWord(op);
}

void TokenDictionary::deleteWord(const char & c) {
    dictionary->deleteWord(c);
}

void TokenDictionary::deleteWord(const std::string & s) {
    dictionary->deleteWord(s);
}

void TokenDictionary::gotoNext(const char & c) {
